  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (['complete','failed'].includes(resp.data.status)) clearInterval(interval)
      } catch {
        setStatus('failed')
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-lg p-8 max-w-sm w-full text-center">
        ...
        {status === 'complete' && <p className="text-green-600 font-bold">{t('loggedIn')}</p>}
        {status === 'failed' && <p className="text-red-600 font-bold">{t('errorOccurred', { action: t('login') })}</p>}
      </div>
    </div>
  )
}
